...
export interface ISubzillaAPI {
    ...
    onProcessingProgress: (
        callback: (progress: { current: number; total: number; currentFile?: string; stats: IBatchStats }) => void,
    ) => void;
...
const api: ISubzillaAPI = {
    ...
    onProcessingProgress: (
        ...
        ipcRenderer.on('processing-progress', (_, progress) =>
            callback(progress as { current: number; total: number; currentFile?: string; stats: IBatchStats }),
        );
    },
    onUpdateDownloadProgress: (
        callback: (progress: { percent: number; transferred: number; total: number }) => void,
        ...
        ipcRenderer.on('update-download-progress', (_, progress) =>
            callback(progress as { percent: number; transferred: number; total: number }),
        );
    },
...
// Only expose API when not in test environment
